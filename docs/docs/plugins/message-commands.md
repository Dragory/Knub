...
export const ExampleCmd = guildPluginMessageCommand({
  ...
  signature: {
    user: ct.user({ required: false }),
    text: ct.string({ required: false }),
  },
...
Parameter types are defined using `commandTypeHelpers` (or `baseCommandParameterTypeHelpers`). Each helper accepts a config object such as `{ required: false }`.
...
signature: {
  user: ct.user({ required: false }),
  reason: ct.string({ required: false }),
}
```
